...
The system SHALL provide a deterministic dummy timeline for the dummy account so visual verification remains possible even when live data does not currently contain all supported post types.

#### Scenario: Dummy timeline loads supported post variants
- **WHEN** an authenticated dummy account opens the browse timeline
- **THEN** the system returns a deterministic set of dummy posts that covers every post type currently supported by timeline rendering
...
The system SHALL include stable remote media URLs for dummy posts that represent supported image, video, and GIF-style embeds.

#### Scenario: Remote media fixtures cover supported media types
- **WHEN** the dummy timeline or detail page renders a post with an image, video, or GIF-style embed
- **THEN** the system provides a stable remote media URL for that embed variant
