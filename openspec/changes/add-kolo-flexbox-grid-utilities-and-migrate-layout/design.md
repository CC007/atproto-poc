...
Keep flex and grid together as one layout concern, but separate their token models internally if that improves parsing clarity. Support only documented values actually used by BlueArt plus complete variants for every adopted subfamily; retain arbitrary template expressions as exceptions.
